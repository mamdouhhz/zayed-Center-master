...
export const usePatientsStore = defineStore("patients", () => {
    ...
    try {
      const { data } = await ApiService.query("patients", {
        params: {
          ...
          ...(params.value.sort && { inClass: true }),
        },
      });
  ...
  async function uploadImage(photo: any, _id: any) {
    try {
      const response = await ApiService.post(`patients/${_id}/photo`, photo);
      return response;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  async function deletePhoto(id: any) {
    try {
      return await ApiService.delete(`patients/${id}/photo`);
    } catch (error) {
      console.error(error);
      throw error;
    }
